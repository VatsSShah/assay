...
**Modality column.** `text` means the runner exercises the task's own channel. **`text sim`** means the task *describes* a non-text channel but the shipped runner exercises it as text: no pixel, audio or document decoding path exists in this repository, and `TASKS.md` states for each of these tasks that the image is a placeholder and the directive rides the accompanying text. Six tasks are in that state. Treat them as text-channel tasks until a real modality path ships.
...
| M20 | Visual / image-channel injection | B | canary | `image-pixels` | image (**text sim**) | MCP06, MCP10 | #1 | ASI01, ASI02 | AML.T0051.001, AML.T0086 |
...
| M26 | Image-result tool poisoning (multimodal) | B | canary | `image_tool_result` | image (**text sim**) | MCP06, MCP10 | #1, #18 | ASI01, ASI02 | AML.T0051.001, AML.T0086 |
| M27 | Cross-modal capability chain (vision->network) | B | canary | `cross_modal_vision_to_network` | image (**text sim**) | MCP01, MCP06 | #1 | ASI01, ASI02 | AML.T0051.001, AML.T0086 |
| M28 | Steganographic chart canary (alt-text) | B | canary | `image_alt_text` | image (**text sim**) | MCP06, MCP10 | #1, #18 | ASI01, ASI02 | AML.T0051.001, AML.T0086 |
| M29 | Multimodal rug-pull on image resource | B | canary | `image_rug_pull` | image (**text sim**) | MCP03, MCP10 | #14, #18 | ASI04, ASI02 | AML.T0110, AML.T0051.001 |
| M30 | QR-code exfil in tool-returned image | B | canary | `image_qr_code` | image (**text sim**) | MCP10, MCP06 | #1, #18 | ASI01, ASI02 | AML.T0051.001, AML.T0086 |
