...
def load_frozen_catalog(path: str | None = None) -> dict | None:
    ...
    candidates += [here / "tasks.json", here / "assay_bench" / "data" / "tasks.json",
                   Path.cwd() / "tasks.json"]
...
def _check_structure(manifest) -> None:
    ...
    optional = ("scope", "provenance", "precommitment", "attestation")
...
def _check_scores(manifest) -> None:
    findings = manifest["findings"]
    recomputed = {"agent_resistance_score": _resistance([f for f in findings if f["mode"] == "B"]),
